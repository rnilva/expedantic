...
def example_training_loop():
    ...
    for epoch in range(3):
        ...
        for batch in range(10):
            ...
            # logger.gradient_norm.log(np.random.exponential(1.0))
            logger.batch_time.log(0.1 + np.random.random() * 0.05)

            # if np.random.random() > 0.8:
            #     logger.errors.log()  # Just count
        ...
        print(f"  Errors: {entry.get('errors', 0)}")

    return logger


if __name__ == "__main__":
    logger = example_training_loop()
    print(logger.to_dataframe())


def example_custom_logger():
    """Show how easy it is to create custom loggers"""
