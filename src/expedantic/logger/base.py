...
class LoggerBase:
    ...
    def __init__(self, 
                 name: str | None = None,
                 sinks: list[SinkProtocol] | None = None) -> None:
        ...
        if items:
            ...
            for sink in self.sinks:
                try:
                    ...
                    import warnings
                    warnings.warn(f"Sink {sink.__class__.__name__} failed to write: {e}", RuntimeWarning)
    ...
    def close(self) -> None:
        ...
        for sink in self.sinks:
            try:
                ...
                import warnings
                warnings.warn(f"Error closing sink {sink.__class__.__name__}: {e}", RuntimeWarning)
        self.sinks.clear()
